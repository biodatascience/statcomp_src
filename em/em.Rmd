...
pairs(mat, panel = function(...) smoothScatter(..., nrpoints = 0, add = TRUE), gap = 0.2)
```
...
### now start the EM algorithm
  ...
  while(eps > tol & iter < maxit){
    ...
    ## start M-step (Sec. 2.2.4)
      ...
      for(k in 1:K) fit[[k]] = glm(y ~ X -1, family = poisson(), weights = pp[,k])
    
      
    ## start E-step (Sec. 2.2.3)
      # calculate numerator, fitted values are exp(X %*% beta_hat)
      for(k in 1:K) pp[,k] = pi[k]*dpois(y,lambda = fit[[k]]$fitted)
    ...
    ## calculate LL (Sec 2.2.1)
      ...
      for(i in 1:K) interior_sum = interior_sum + pi[k]*dpois(y,lambda = fit[[k]]$fitted) 
