...
Gaussian quadrature is very effective for integrals of the following form: $$ \int_a^b f(x)w(x)dx$$ and  $$ \int_a^b x^pw(x)dx < \infty.$$  You may recall that the former may be relevant in Bayesian applications where we have a density function $f(x)$ and some prior $w(x)$.  
...
&\approx \sum_{k = 1}^M \frac{1}{\sqrt{\pi}}\prod_{j = 1}^{5}f(y_{1j} | \lambda_{1j})
\end{aligned}

where now $\log(\lambda_{1j}) = \boldsymbol{x}_{1j}^T\boldsymbol{\beta} + \sigma_{\gamma}\sqrt{2}x_k$.  Let’s try it out:
...
$$ \int h(\boldsymbol{x})f(\boldsymbol{x})dx \approx \frac{1}{M}\sum_{k = 1}^M h(\boldsymbol{X}_k),$$ where $\boldsymbol{X}_1,\ldots,\boldsymbol{X}_M$ are an iid samples drawn from $f(\boldsymbol{x})$. As $M \rightarrow \infty$, $\frac{1}{M}\sum_{k = 1}^M h(\boldsymbol{X}_k) \rightarrow  \int h(\boldsymbol{x})f(\boldsymbol{x})dx$ by the strong law of large numbers.  
...
If we let $$\hat{\mu}_{MC} = \frac{1}{M}\sum_{k = 1}^M h(\boldsymbol{X}_k)$$ and assume that $\hat{\mu}_{MC} \rightarrow \mu = \int h(\boldsymbol{x})f(\boldsymbol{x})dx$ as $M$ \rightarrow \infty, and $var(\hat{\mu}_{MC}) = E[(h(\boldsymbol{x}) - \mu)^2] = \sigma^2$, then using results from BIOS 760 we know that the sampling variance for $\hat{\mu}_{MC} = \sigma^2/M$.  Here $\mu$ is the expectation of $h(\boldsymbol{x})$.

Given this, we can calculate the estimate for $\sigma^2$ using the following expression, which is similar to the form of the sample variance that you have seen in your other classes: $$\hat{var}(\hat{\mu}_{MC}) =\frac{1}{M-1} \sum_{k = 1}^M [h(\boldsymbol{X}_k) - \hat{\mu}_{MC}]^2$$.
...
Here we wish to compute the following $$\int Yf(Y)dY.$$  Using what we have already discussed, we know that we can approximate the expected value by generating $M$ samples $Y_1,\ldots,Y_M$ from the $Po(5)$ distribution, and then calculating $\frac{1}{M}\sum_{k=1}^M h(Y_k) = \frac{1}{M}\sum_{k=1}^M Y_k$.
...
So far, we have introduced the MC estimator for the integral  $\hat{\mu}_{MC} = \frac{1}{M}\sum_{k = 1}^M f(\boldsymbol{X}_k)$.  However, alternative strategies may be employed to reduce the variance of the MC estimator.  The side benefit of these approaches is that one may be able to achieve sufficient accuracy with a fewer number of samples. We will  connect this approach with the AGQ approach discussed above. 
...
We can see that this probability is small as expected.  We can also use MCI to obtain this estimate as well.  Here, we are trying to compute $E[I[Y > 2]] = \int I[Y > 4]f(Y)dY = p(Y > 4)$, where $f(Y) \sim N(0,1)$.  Here we can approximate the integral such that $\int I[Y > 4]f(Y)dY = \sum_{k = 1}^{M}I[Y_k > 4]/M$ Let’s try to compute this value using $k = 1,000$ samples:
...
where $g(\boldsymbol{x})$ is the alternative distribution that we are sampling from, sometimes called the "importance sampling function" or "envelope".  Using similar arguments as before, we can estimate $\mu$ in this case via $$\hat{\mu}_{IS}^* = \sum_{k =  1}^M h(\boldsymbol{X}_k)w^*(\boldsymbol{X}_k),$$ where  $w^*(\boldsymbol{X}_k) = \frac{f(\boldsymbol{X}_k)}{g(\boldsymbol{X}_k)}$ can be thought of as weights, and $\boldsymbol{X}_1,\ldots,\boldsymbol{X}_M$ are an iid sample from $g$.   For this approach to work well, it must be easy to sample from $g$, and $f$ must be easy to evaluate, otherwise you are just adding more computational work to evaluate this integral.  
