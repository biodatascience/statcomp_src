...
We perform separate integrals for each $\gamma_i$, 22 in total, which share a common distribution determined by $\sigma^2_gamma$.  In some sense, we can interpret these $\gamma_i's$ was 22 iid draws from $\phi(\gamma_i| 0, \sigma^2_{\gamma})$, which reflects the baseline subject-level variability about $\beta_0$.  This likelihood can be represented as the following:
...
```{r}
# yi is a vector pertaining to single individual i
...
f_gammai = function(gammai, yi, beta, s2gamma) {
