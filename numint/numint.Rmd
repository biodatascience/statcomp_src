...
This is helpful for integrating joint probability distributions, such as in our GLMM examples ($f(\x)$ in this setting is $\phi(\gamma_i)$, or in calculating expectations of functions of $x$ emprically ($E[h(\x)]$).  

Due to the random nature of the nodes, some variance in the approximation is expected.  This is called Monte Carlo Variance (MCV) and is larger at smaller $M$.  If we let $\hat{\mu}_{MC} = \frac{1}{n}\sum_{m = 1}^M f(\X_i)$ and assume that $\hat{\mu}_{MC} \rightarrow \mu = \int h(\x)f(\x)dx$, and $var(\x) = E[(h(\x) - \mu)^2] = \sigma^2$, then using results from BIOS 760 we know that the sample variance for $\hat{\mu}_{MC} = \sigma^2/n$.  Here $\mu$ is the expectation of $h(\X)$.
