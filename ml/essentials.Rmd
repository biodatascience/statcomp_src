...
be found in the help page for `?trainControl` under `method`. We will
also cover some of the most popular methods below.
...
# Feature selection using the outcome must occur within the loop
...
```{r}
tg <- expand.grid(NumVars=c(4,10,20),lambda=c(0,1,10,100))
head(tg)
trCtl <- trainControl(method="boot", number=50)
fit <- train(x, y, method="sparseLDA", tuneGrid=tg, trControl=trCtl)
ggplot(fit, metric="Kappa") + ylim(0.7,1)
```
