...
We remove a single troublesome line from the file using `grep`:

```
grep -v 8584297742 aps_failure_training_set.csv > aps_clean.csv
```
...
```{r}
library(readr)
dat <- read_csv("aps_clean.csv", skip=20, na="na")
table(dat$class)
...
here makes use of the Gini impurity.
...
```{r}
trCtl <- trainControl(savePredictions=TRUE, classProbs=TRUE)
...
costFn(cut=.5, fit$pred)
s <- seq(from=0, to=0.5, length=100)
costs <- sapply(s, costFn, df=fit$pred)
plot(s, costs, type="l", lwd=3, col="dodgerblue")
s <- seq(from=0, to=0.1, length=100)
costs <- sapply(s, costFn, df=fit$pred)
plot(s, costs, type="l", lwd=3, col="dodgerblue")
```
...
with(dat, plot(x,y))
rfit <- rpart(y ~ x, data=dat, method="anova", cp=.001)
rpart.plot(rfit)
```

```{r}
pred <- predict(rfit, dat)
with(dat, plot(x,y))
...
rf.fit <- train(dat["x"], dat$y, method="rf", trControl=trCtl, tuneGrid=tg)
with(dat, plot(x,y))
...
```{r}
trCtl <- trainControl(method="cv", number=5, savePredictions=TRUE)
kfit <- train(dat["x"], dat$y, method="knn", trControl=trCtl)
ggplot(kfit)
with(dat, plot(x,y))
