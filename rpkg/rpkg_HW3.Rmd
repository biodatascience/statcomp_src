...
variable ridge parameter `lambda`. That is, use `lambda[1]` for the
first column, `Y[,1]` and so on. Call your R wrapper function
`colRidge2`. Compare the output to `colRidge1`. It is not expected to
be numerically identical, but it should be "close" which you can see
from the plots. How much faster is the C++ version?

Hint: use the simple ridge formula:
