...
Now here is the higher level function that loops over each subject. 
...
One interesting thing to note is that the MH ratio can be interpreted as a ratio of importance weights, where if $w^∗ = f(X^∗)/g(X^∗)$ and $w^{(t)} = f(x^{(t)})/g(x^{(t)})$, then $R(x^{(t)},X^∗)= w^∗/w^{(t)}$.  This suggests that if the prior draw results in a larger than usual $w^{(t)}$ than typical $w^∗$ values, the chain may get stuck for a while at $x^{(t)}$ (large denominator --> small ratio --> low acceptance probability of new proposals). 

As a result, the criteria for choosing importance sampling distributions are also relevant to choosing good proposal distributions.  We will talk more about this after some of the following code examples. 
...
# note that since the prior is uniform it is not specified in f below (enters as a constant)
f = function(x,y){
...
# note that we specify g here just for completeness (it cancels out with the uniform prior as mentioned earlier)
...
# notice that we do not specify the prior here or in the MH ratio below as it is uniform
...
for (i in 1:(M-1)) {
  ...
  # generate proposal in terms of u
	...
	# calculate MH ratio on the log scale.  Two ways to do this:
	
	# 1) logR = ( log.lik(pi*)  + log(g(logit(pi^{(t)}) | logit(pi*))) + log(|J(pi^{(t)}|) )
	#          - ( log.lik(pi^{(t)})  + log(g(logit(pi*) | logit(pi^{(t)}))) + log(|J(pi*)|) )
	# where |J| = 1/(p(1-p)), since u = logit(p/(1-p)).  g's are uniform here/
	# This follows eq 7.7 in the book (p. 208).  Alternatively, can use 
	
	# 2) logR = (log.lik(e^u*/1+e^u*)  + log(g(u^{(t)} | u*) + log(|J(u*)|)) )
	#          - (log.lik(e^{u^{(t)}}/1+e^{u^{(t)}})  + log(g(u* | u^{(t)})) + log(|J(u^{(t)})|)
	# where |J| = exp(u)/(1+exp(u))^2, since pi = exp(u)/(1+exp(u)). g's are uniform here.
	# This follows eq 7.8 in the book (p. 208).  
	
	# Either way ends up being equivalent - we use 2) below
	
  logR = (log.like(exp(u[i+1])/(1+exp(u[i+1])), y) + u[i+1] - log((1 + exp(u[i+1]))^2) ) - ( log.like(exp(u[i])/(1+exp(u[i])), y) + u[i] - log((1 + exp(u[i]))^2) )
...
That is, the proposal density here is exactly the conditional target density (which is known here).  Therefore, the Metropolis–Hastings ratio equals 1 for this update, which means that the candidate is always accepted. We will see in the next lecture an example of Metropolis-within-gibbs, where none of the conditional densities are known and are approximated with separate metropolis steps instead. In that scenario the proposal density is not equal to the target density (since the conditional density not known in advance), and therefore the MH ratio will not always equal 1 as in the example above. 
...
after burn-in can be estimated using $L/\hat{\tau}$.
...
several platforms. A popular application mode is to use BUGS within the R statistical
