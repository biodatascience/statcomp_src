...
&\approx \frac{1}{M}\sum_{k = 1}^M\left[\sum_{j = 1}^{5}\log\left(f(y_{ij} | \lambda_{ijk})\right) + \log\left(\phi(X_k| 0, \Sigma_{\gamma})\right)\right]
\end{align}
...
For the sake of illustration/speed we fixed $M$ to be relatively small and terminated the algorithm after 35 iterations.  In reality we may need a much larger M and more EM iterations to reach convergence. 
