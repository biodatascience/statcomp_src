...
In the current problem, the MLE for $\mu$, $\hat{\mu} = \hat{\y}$, is then obtained  by setting the 1st derivative of the log-likelihood (also known as the "score function") equal to 0 and then solving for $\mu$ (can you show this?).  In other words, we find the root of the score function with respect to $\mu$.  For the regression based approach, we are simply minimizing the residual sum of squares (RSS), defined as $\sum_{i = 1}^{100} (y_i - \mu)^2$.
...
* Cons
    ...
    + In the multivariate setting, need checks on hessian to avoid singularities, other issues
