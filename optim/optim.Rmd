...
We can also rexpress the original form of this problem as an intercept-only linear regression model:  $$y_i = \X\beta + \epsilon_i$$ where $\epsilon_i \sim N(0,\sigma^2)$, $\X$ is simply a column vector of ones, and $\beta = \mu$, a scalar parameter in this setting.  Again,  $\sigma^2$ is known in this case.   Given this, we can show that an estimate for $\beta$ ($\mu$) is also $\bar{\mathbf{y}}$. Based on the normal equations from the linear model framework, we have closed form solution $\hat{\beta} = (\X'\X)^{-1}\X'\y$,  which, plugging in $\X$ and $\y$ from this situation, also yields $\bar{\y}$.  
...
Here,  $$f'(\theta) = \frac{1+x - x\log(x)}{x(1+x)^2}.$$ So...not easy to solve for.
...
We can see this is a nice form and avoids having to analytically determine a second derivative.  
```{r, echo=TRUE}
...
* Pros
    ...
    + Can use off the shelf applications of weighted least squares
* Cons
...
print(end - start)
```
Looks like the starting point was not good, but it recovered.  Now lets check the result against R's glm function
```{r, echo=TRUE}
print(beta)
...
Takes more iterations and time than the prior methods, but is not surpising since it does not utilized or approximate the hessian.  We could drop this time further by increasing alpha or the convergence threshold, but at the expense of accuracy/convergence.
